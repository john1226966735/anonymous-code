...
class DataLoader:
    def __init__(self, task_dir, plan_emb_dir=None):
        ...
        if task_dir.startswith('MetaQA'):
            self.fact_triple = self.read_metakb('../data/MetaQA/kb.txt')
            self.train_data = self.read_qa(self.task_dir + '/ntm/qa_train.txt')
        ...
        # ---- PlanR addition: load plan embeddings ----
        self.plan_emb = None
        self.plan_nsteps = None
        if plan_emb_dir is not None:
            self.load_plan_emb(plan_emb_dir)

    def load_plan_emb(self, emb_dir):
        """Load plan embeddings and nsteps arrays.

        Plan embeddings are stored per-split and concatenated in the same order
        as question IDs: [train, valid, test].
        """
        if 'webqsp' in self.task_dir:
            prefix = 'webqsp'
        elif 'CWQ' in self.task_dir:
            prefix = 'CWQ'
        else:
            print('Plan embeddings not supported for this dataset, skipping.')
            return

        parts_emb = []
        parts_ns = []
        for split in ['train', 'valid', 'test']:
            plan_path = os.path.join(emb_dir, f'{prefix}-{split}-plan.npy')
            ns_path = os.path.join(emb_dir, f'{prefix}-{split}-nsteps.npy')
            if not os.path.exists(plan_path) or not os.path.exists(ns_path):
                print(f'Warning: plan embedding not found: {plan_path}, skipping plan loading.')
                return
            parts_emb.append(np.load(plan_path))
            parts_ns.append(np.load(ns_path))

        self.plan_emb = np.concatenate(parts_emb, axis=0)  # (N_total, max_steps, emb_dim)
        self.plan_nsteps = np.concatenate(parts_ns, axis=0)  # (N_total,)
        print(f'Plan embeddings loaded: {self.plan_emb.shape}, nsteps: {self.plan_nsteps.shape}')

    # ---- Original DualR methods below (unchanged) ----
    ...
    def get_neighbors(self, nodes, qids=None):
        if 'webqsp' in self.task_dir or 'CWQ' in self.task_dir:
            ...
            for batch_idx, qid in enumerate(qids):
                batch_mask = nodes[:, 0] == batch_idx
                batch_nodes = nodes[batch_mask]
                if len(batch_nodes) == 0:
                    continue

                # Keep each question strictly inside its own retrieved subgraph.
                # Mixing all qids in one KG allows same-head entities from another
                # question to leak edges into the current question's search space.
                n_ent = len(batch_nodes)
                kg = np.concatenate([
                    batch_nodes[:, 1].reshape(-1, 1),
                ...
                n = len(kg)
                m_sub = csr_matrix((np.ones((n,)), (np.arange(n), kg[:, 0])),
                                   shape=(n, self.n_ent))
                node_1hot = csr_matrix(
                    (np.ones(len(batch_nodes)), (batch_nodes[:, 1], np.zeros(len(batch_nodes), dtype=np.int64))),
                    shape=(self.n_ent, 1)
                )
                edge_1hot = m_sub.dot(node_1hot)
                edge_rows = np.nonzero(edge_1hot)[0]
                sampled_edges.append(
                    np.concatenate([
                        np.full((len(edge_rows), 1), batch_idx, dtype=np.int64),
                        kg[edge_rows]
                    ], axis=1)
                )

            if sampled_edges:
                sampled_edges = np.concatenate(sampled_edges, axis=0)
            else:
                sampled_edges = np.zeros((0, 4), dtype=np.int64)
            sampled_edges = torch.LongTensor(sampled_edges).cuda()
        else:
            KG = self.KG
            M_sub = self.M_sub
            node_1hot = csr_matrix((np.ones(len(nodes)), (nodes[:, 1], nodes[:, 0])),
                                   shape=(self.n_ent, nodes.shape[0]))
            edge_1hot = M_sub.dot(node_1hot)
